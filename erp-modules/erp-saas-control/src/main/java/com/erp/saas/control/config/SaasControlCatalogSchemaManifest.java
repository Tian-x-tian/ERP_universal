...
public final class SaasControlCatalogSchemaManifest {
    ...
    public static Map<String, Table> tables() {
        ...
        add(tables, "saas_provisioning_task", "task_id:bigint:N:~:~:~;request_id:varchar(128):N:~:~:utf8mb4_unicode_ci;request_hash:char(64):N:~:~:utf8mb4_unicode_ci;tenant_id:varchar(20):N:~:~:utf8mb4_unicode_ci;plan_id:bigint:N:~:~:~;status:varchar(16):N:~:~:utf8mb4_unicode_ci;attempt_count:int:N:0:~:~;lease_until:datetime(3):Y:~:~:~;tenant_record_id:bigint:Y:~:~:~;company_id:bigint:Y:~:~:~;dept_id:bigint:Y:~:~:~;role_id:bigint:Y:~:~:~;user_id:bigint:Y:~:~:~;activation_expires_at:datetime(3):Y:~:~:~;last_error_type:varchar(128):Y:~:~:utf8mb4_unicode_ci;create_by:varchar(64):N:~:~:utf8mb4_unicode_ci;create_time:datetime(3):N:~:~:~;update_by:varchar(64):N:~:~:utf8mb4_unicode_ci;update_time:datetime(3):N:~:~:~;version_no:bigint:N:0:~:~",
                "PRIMARY:U:task_id;uk_saas_provisioning_request:U:request_id;uk_saas_provisioning_tenant:U:tenant_id;idx_saas_provisioning_status_lease_update:N:status,lease_until,update_time",
                "ck_saas_provisioning_status=status IN ('PENDING', 'PROVISIONING', 'INITIALIZED', 'SUCCEEDED', 'FAILED');ck_saas_provisioning_attempt_count=attempt_count >= 0;ck_saas_provisioning_lease=((status = 'PROVISIONING') AND (lease_until IS NOT NULL)) OR ((status <> 'PROVISIONING') AND (lease_until IS NULL));ck_saas_provisioning_result=((status IN ('PENDING', 'PROVISIONING')) AND (tenant_record_id IS NULL) AND (company_id IS NULL) AND (dept_id IS NULL) AND (role_id IS NULL) AND (user_id IS NULL) AND (activation_expires_at IS NULL)) OR ((status IN ('INITIALIZED', 'SUCCEEDED')) AND (tenant_record_id IS NOT NULL) AND (company_id IS NOT NULL) AND (dept_id IS NOT NULL) AND (role_id IS NOT NULL) AND (user_id IS NOT NULL) AND (activation_expires_at IS NOT NULL)) OR ((status = 'FAILED') AND (((tenant_record_id IS NULL) AND (company_id IS NULL) AND (dept_id IS NULL) AND (role_id IS NULL) AND (user_id IS NULL) AND (activation_expires_at IS NULL)) OR ((tenant_record_id IS NOT NULL) AND (company_id IS NOT NULL) AND (dept_id IS NOT NULL) AND (role_id IS NOT NULL) AND (user_id IS NOT NULL) AND (activation_expires_at IS NOT NULL))))");
