-- 20260322_03: workflow 服务补齐绑定表与员工审批流程默认配置
...
SET `process_name` = '员工档案审批流程',
    `remark` = CASE
      ...
      WHEN `remark` = 'workflow 初始化员工主数据审批流程定义' THEN 'workflow 初始化员工档案审批流程定义'
      WHEN `remark` = 'workflow 增量脚本补齐员工主数据审批流程定义' THEN 'workflow 增量脚本补齐员工档案审批流程定义'
      ELSE `remark`
    END
